...
def test_one_dataset(
    ...
    checkpoint = torch.load(os.path.join(
        'model', params.model, params.save, file_name) + '_'+str(best_epoch),weights_only=False)
...
if __name__ == '__main__':
    ...
    if dataset in {"moocradar_pid"}:
        ...
        params.seqlen = 200
        params.data_dir = 'data/MOOCRadar/akt_format'
        params.data_name = 'moocradar_pid'
        params.n_question = 2513
        params.n_pid = 2513
        params.dropout = 0.2
        params.l2 = 5e-5
    ...
    # 可选：加载与样本严格对齐的语义向量（仅对 moocradar_pid 生效）
    train_semantic = None
    valid_semantic = None
    test_semantic = None
    if dataset in {"moocradar_pid"}:
        ...
        if os.path.exists(train_sem_path) and os.path.exists(valid_sem_path) and os.path.exists(test_sem_path):
            ...
            with open(valid_sem_path, "rb") as f:
                valid_semantic = pickle.load(f)
            with open(test_sem_path, "rb") as f:
                test_semantic = pickle.load(f)
            # 统一成 numpy 数组，确保 run.py 中 torch.from_numpy 可用
            if not isinstance(train_semantic, np.ndarray):
                train_semantic = np.asarray(train_semantic, dtype=np.float32)
            if not isinstance(valid_semantic, np.ndarray):
                valid_semantic = np.asarray(valid_semantic, dtype=np.float32)
            if not isinstance(test_semantic, np.ndarray):
                test_semantic = np.asarray(test_semantic, dtype=np.float32)
            # 要求语义数据与样本数一致
            if (len(train_semantic) != train_q_data.shape[0] or
                    len(valid_semantic) != valid_q_data.shape[0]):
                raise ValueError(
                    f"语义向量样本数与 AKT 输入不一致："
    ...
    test_q_data, test_qa_data, test_index = dat.load_data(
        test_data_path)
    if dataset in {"moocradar_pid"} and test_semantic is not None:
        if len(test_semantic) != test_q_data.shape[0]:
            raise ValueError(
                f"test 语义向量样本数 {len(test_semantic)} 与 test_q_data {test_q_data.shape[0]} 不一致"
            )
    test_one_dataset(
        params,
        file_name,
