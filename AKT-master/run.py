...
def train(net, params, optimizer, q_data, qa_data, pid_data, label, semantic_data=None):
    net.train()
    pid_flag, model_type = model_isPid_type(params.model)
    N = int(math.ceil(len(q_data) / params.batch_size))
    q_data = q_data.T  # Shape: (seqlen, num_samples)
    qa_data = qa_data.T  # Shape: (seqlen, num_samples)

    # Shuffle the data (按样本维度打乱)
    ...
    # 语义向量按样本维度同步打乱，保持与 q_data/qa_data 对齐
    if semantic_data is not None:
        if len(semantic_data) != q_data.shape[1]:
            raise ValueError(
                f"semantic_data 样本数 {len(semantic_data)} 与 q_data 列数 {q_data.shape[1]} 不一致"
            )
        # semantic_data: [num_samples, seqlen, semantic_dim]
        semantic_data = semantic_data[shuffled_ind]
    ...
    for idx in range(N):
        ...
        # 对应 batch 的语义向量（若启用），形状 [bs, seqlen, semantic_dim]
        if semantic_data is not None:
            sem_one_seq = semantic_data[idx*params.batch_size:(idx+1)*params.batch_size]
        ...
        # 若有语义向量，则构造对应的 tensor（形状 [bs, seqlen, semantic_dim]）
        if semantic_data is not None:
            input_sem = torch.from_numpy(sem_one_seq).float().to(device)
        else:
            input_sem = None

        if pid_flag:
            input_pid = torch.from_numpy(input_pid).long().to(device)
            if input_sem is not None:
                loss, pred, true_ct = net(input_q, input_qa, target, input_pid, semantic_inputs=input_sem)
            else:
                loss, pred, true_ct = net(input_q, input_qa, target, input_pid)
        else:
            if input_sem is not None:
                loss, pred, true_ct = net(input_q, input_qa, target, semantic_inputs=input_sem)
            else:
...
def test(net, params, optimizer, q_data, qa_data, pid_data, label, semantic_data=None):
    ...
    # 语义向量与样本对齐（测试阶段不打乱顺序）
    if semantic_data is not None and len(semantic_data) != q_data.shape[1]:
        raise ValueError(
            f"semantic_data 样本数 {len(semantic_data)} 与 q_data 列数 {q_data.shape[1]} 不一致"
        )
    ...
    for idx in range(N):
        ...
        with torch.no_grad():
            if pid_flag:
                if input_sem is not None:
                    loss, pred, ct = net(input_q, input_qa, target, input_pid, semantic_inputs=input_sem)
                else:
                    loss, pred, ct = net(input_q, input_qa, target, input_pid)
            else:
                if input_sem is not None:
                    loss, pred, ct = net(input_q, input_qa, target, semantic_inputs=input_sem)
                else:
