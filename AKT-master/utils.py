...
def load_model(params):
    ...
    if model_type in {'akt'}:
        model = AKT(n_question=params.n_question, n_pid=params.n_pid, n_blocks=params.n_block, d_model=params.d_model,
                    dropout=params.dropout, kq_same=params.kq_same, model_type=model_type, l2=params.l2,
                    input_semantic_dim=getattr(params, "input_semantic_dim", 0),
                    use_early_fusion=bool(getattr(params, "use_early_fusion", False))).to(device)
    else:
        model = None
    return model
