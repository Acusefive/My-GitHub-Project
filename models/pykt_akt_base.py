...
class TransformerLayer(nn.Module):
    ...
    def forward(self, mask, query, key, values, apply_pos=True, pdiff=None):
        ...
        diagonal = 0 if mask == 1 else -1
        src_mask = torch.tril(
            torch.ones((1, 1, seqlen, seqlen), device=query.device, dtype=torch.bool),
            diagonal=diagonal,
        )
...
class MultiHeadAttention(nn.Module):
    ...
    def pad_zero(self, scores, bs, dim, zero_pad):
        if zero_pad:
            ...
            pad_zero = torch.zeros(bs, 1, dim, device=scores.device, dtype=scores.dtype)
...
def attention(q, k, v, d_k, mask, dropout, zero_pad, gamma=None, pdiff=None):
    ...
    attn_device = scores.device
    device = attn_device
    mask = mask.to(attn_device)
    mask_fill_value = torch.finfo(scores.dtype).min
    x1 = torch.arange(seqlen, device=attn_device).expand(seqlen, -1)
    x2 = x1.transpose(0, 1).contiguous()

    with torch.no_grad():
        scores_ = scores.masked_fill(mask == 0, mask_fill_value)
        scores_ = F.softmax(scores_, dim=-1)  # BS,8,seqlen,seqlen
        scores_ = scores_ * mask.to(device=device, dtype=scores.dtype) # 结果和上一步一样
        distcum_scores = torch.cumsum(scores_, dim=-1)  # bs, 8, sl, sl
        disttotal_scores = torch.sum(
            scores_, dim=-1, keepdim=True)  # bs, 8, sl, 1 全1
        # print(f"distotal_scores: {disttotal_scores}")
        position_effect = torch.abs(
            x1-x2)[None, None, :, :].to(device=device, dtype=scores.dtype)  # 1, 1, seqlen, seqlen 位置差值
    ...
    # Compute the decay in float32 so the original 1e5 clamp remains valid
    # under FP16 training, then cast it back before applying it to scores.
    dist_scores = dist_scores.float()
    gamma = gamma.float()
    ...
    scores = scores * total_effect.to(dtype=scores.dtype)

    scores.masked_fill_(mask == 0, mask_fill_value)
    scores = F.softmax(scores, dim=-1)  # BS,8,seqlen,seqlen
    # print(f"before zero pad scores: {scores.shape}")
    # print(zero_pad)
    if zero_pad:
        pad_zero = torch.zeros(bs, head, 1, seqlen, device=attn_device, dtype=scores.dtype)
